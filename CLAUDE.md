...
- `.claude-plugin/marketplace.json` and everything under `plugins/` — regenerated by `pnpm skills:marketplace` and **committed in the same change** as any skill edit. Every skill ships in two plugins: one per `(domain, class)` (`skills-master-apple-code`) and one per `(domain, class, category)` (`skills-master-apple-code-swiftui-views`), because a class-sized install overruns the agent's always-on listing budget — see `docs/architecture.md`. A class with a single category gets no category plugin, since it would duplicate the class one. CI fails on drift (`marketplace build --check`). The build prunes output it no longer owns, so a skill that moves class does not keep shipping from its old plugin. Release CI rebuilds and zips these as a release asset.
...
**A push to `main` publishes only when it can change the published package** — that is, when it touches `packages/cli/**` (excluding `test/`), `pnpm-lock.yaml`, or the root `package.json`. `.github/workflows/release.yml` reruns CI, then computes the next version from the latest `v*` git tag plus the head commit message — `#major` > `#minor` > default patch — writes it into `packages/cli/package.json` on the runner only (git tags are the source of truth, nothing is committed back), publishes to npmjs **and** GitHub Packages, then tags and creates the release, and finally asserts that npmjs, the tag and the Release all agree. Include `#minor`/`#major` in a merge commit message when the change warrants it. `workflow_dispatch` releases on demand.

**Content merges do not need a release, and no longer trigger one.** The npm package is content-free (`files` is `dist`, `README.md`, `LICENSE`), so skills never travel through it: the CLI fetches them at runtime from `github:iChintanSoni/skills-master/skills#main`, and the Claude marketplace reads the committed `.claude-plugin/` + `plugins/` off `main`. Both go live the moment a PR merges. Publishing on every push shipped mostly nothing — `0.3.9` through `0.3.17` are byte-identical apart from the embedded version string, and one day in August produced 30 releases.
