...
- `docs/taxonomy.md` — regenerate with `node scripts/gen-taxonomy.mjs` after the registry (`--check` reports drift). Not gated by CI, so it silently rots if you skip it.
- `.claude-plugin/marketplace.json` and everything under `plugins/` — regenerated by `pnpm skills:marketplace` (one plugin per `(domain, class)`, e.g. `skills-master-apple-code`). Release CI rebuilds and zips these as a release asset.

## CI and release

PRs run two jobs: CLI typecheck + tests, and skills lint + registry drift check. All must pass.
