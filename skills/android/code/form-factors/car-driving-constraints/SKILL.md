...
The `user-notifications` skill covers how to surface heads-up alerts correctly on Android Auto — media and messaging notifications follow different distraction rules than in-app templates. For Media session integration on the car head unit, the `avfoundation-playback` analog on Android is `MediaSessionCompat` combined with `MediaBrowserServiceCompat`, which are prerequisites for a media car app. For deep-linking from a car template into a phone-side activity (Android Auto only), consult the `navigation-architecture` skill for back-stack management.
