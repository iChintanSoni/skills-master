...
- **Android 17 (API 37):** the system attempts autonomous background re-pairing when a bond is lost — `ACTION_KEY_MISSING` now broadcasts only if that re-pairing fails, and `ACTION_PAIRING_REQUEST` carries an `EXTRA_PAIRING_CONTEXT` extra so companion apps can distinguish system-initiated re-pairing from a fresh pairing. Test bond-loss recovery by unpairing on the peripheral side. For RFCOMM (Classic) `BluetoothSocket` streams, apps targeting 37 get `-1` from `InputStream.read()` on disconnect instead of hanging or erroring — make sure read loops terminate on `-1`.
...
- **Worked examples:** [examples.md](examples.md) — read when you want working Bluetooth LE code to adapt
- **Review checklist:** [checklist.md](checklist.md) — run before merging changes to Bluetooth LE
